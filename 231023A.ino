...
void loop() {
    ...
    if(digitalRead(START_KEY) == LOW) { //버튼을 눌렀을 시 아래 코드를 실행
        ...
        bool flag_time= true; //1: LCD 출력, 0: WiFi 재시작
        while(digitalRead(START_KEY) == LOW){
            if(Timer_ButtonPress > 5000) {  //버튼 눌린 시간이 5초 이상인 경우
                flag_time = false; //WiFi 재시작(무선연결)
                break;
            }
        }
        if(flag_time == true){ //버튼을 누르고 5초 이내로 누르지 않은 경우
            lcd.clear();
    ...
    if(Timer_Human > HumanInterval){    //0.1초마다 사람을 감지한다
        Timer_Human = 0;
        if(human_measure == false) cnt_notfound++;
        Read_Human();
